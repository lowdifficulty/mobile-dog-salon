...
export default function SmsBotPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Could not load Licky SMS settings");
  ...
  async function save() {
    ...
    try {
      ...
      setConfig(data.config);
      setTestPhonesText((data.config.testPhones || []).join("\n"));
      setMessage("Licky SMS logic saved");
    } catch (e) {
  ...
  if (loading || !config) {
    return <div className="text-sm text-gray-500">Loading Licky SMS…</div>;
  }

  return (
    <div className="max-w-4xl space-y-5">
      <div>
        <h2 className="text-xl font-bold text-brand">Licky SMS</h2>
        <p className="text-sm text-gray-600 mt-1">
          Licky texts customers from the business number. Keep him in <strong>test mode</strong>{" "}
          until replies look right. In test mode he only sends to allowlisted phones; other
