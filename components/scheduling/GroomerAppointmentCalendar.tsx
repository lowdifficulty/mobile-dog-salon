...
function openSlotsByDate(openSlotKeys: string[]): Map<string, string[]> {
...
export default function GroomerAppointmentCalendar({
  ...
  const seesTeam = groomerSeesTeamAppointments(groomerId);
  ...
  const openByDate = useMemo(() => openSlotsByDate(openSlotKeys), [openSlotKeys]);
  ...
  const selectedOpenSlots = selectedDate ? openByDate.get(selectedDate) ?? [] : [];
  const selectedAppointment = selectedAppointments.find((ap) => ap.id === selectedAppointmentId);
      ...
      <div className="site-card p-6">
        ...
        <div className="grid grid-cols-7 gap-1">
          {monthCells.map((date, index) => {
            ...
            const dayOpenSlots = openByDate.get(date) ?? [];
            const hasBooked = dayAppointments.length > 0;
            const hasOpen = dayOpenSlots.length > 0;
                ...
                <div className="flex gap-1">
                  ...
                  {hasOpen && (
                    <span
                      ...
                      title={`${dayOpenSlots.length} open`}
                    />
                  )}
                </div>
              </button>
        ...
        <p className="text-xs text-gray-500 mt-4">
          Green dot = booked · Pink dot = open slot · Click a day for details.
        </p>
      </div>
              ...
              {selectedAppointments.length === 0 ? (
                ...
                <div className="space-y-2">
                  {selectedAppointments.map((ap) => {
                    const isOwn = ap.groomerId === groomerId;
